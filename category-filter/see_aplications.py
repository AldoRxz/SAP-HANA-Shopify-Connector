...
try:
    ...
    unique_modelo = sorted(df["modelo"].dropna().unique().tolist())
    unique_version = sorted(df["version"].dropna().unique().tolist())
    unique_a_o_inicio = sorted(df["a_o_inicio"].dropna().unique().tolist())
    unique_ano_fin = sorted(df["ano_fin"].dropna().unique().tolist())
    unique_lado = sorted(df["lado"].dropna().unique().tolist())

    # Imprimir las listas de valores únicos
    print("\n🔑 Valores únicos de cada campo:")
    print(f"Partes: {unique_parte}")
    print(f"Marcas: {unique_marca}")
    print(f"Modelos: {unique_modelo}")
    print(f"Versiones: {unique_version}")
    print(f"Años de inicio: {unique_a_o_inicio}")
    print(f"Años de fin: {unique_ano_fin}")
    print(f"Lados: {unique_lado}")
