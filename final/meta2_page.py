...
try:
    ...
    columns = ['codigo', 'parte', 'marca', 'modelo', 'version', 'a_o_inicio', 'ano_fin', 'lado']
    df = pd.DataFrame(results, columns=columns)

    # Asegurar que 'a_o_inicio' y 'ano_fin' sean enteros
    ...
    for _, row in df.iterrows():
        metafields = [
            {"namespace": "custom", "key": key, "value": str(row[key]), "type": "single_line_text_field"} if key not in ['a_o_inicio', 'ano_fin'] else
            {"namespace": "custom", "key": key, "value": int(row[key]), "type": "number_integer"}
            for key in ['parte', 'marca', 'modelo', 'version', 'a_o_inicio', 'ano_fin', 'lado']
        ]
